(function () {
    ...
    ActionPlanController.$inject = ['$scope', '$http', 'Restangular', 'ngTableParams', '$filter', 'cfpLoadingBar', 'toastr'];

    function ActionPlanController($scope, $http, Restangular, ngTableParams, $filter, cfpLoadingBar, toastr) {
        var vm = this;
        $scope.ActionPlan = {
            "ActionPlanId": "",
            "GoalId": "",
        ...
        $scope.LoadStrategyActions = function () {
            if (!($scope.ActionPlan.StrategicDriverId === null)) {
                if (!angular.isUndefined($scope.ActionPlan.StrategicDriverId)) {
        ...
        $scope.LoadRecordsByStrategy = function () {
            ...
            },
            {
                getData: function ($defer, params) {
                    cfpLoadingBar.start();
                    Restangular.all('ActionPlanApi/GetAllActionPlansByStrategicDriverId').getList({
                        pageNo: params.page(),
                        pageSize: params.count(),
                        StrategicDriverId: $scope.ActionPlan.StrategicDriverId.StrategicDriverId
                    }).then(function (businessUnits) {
