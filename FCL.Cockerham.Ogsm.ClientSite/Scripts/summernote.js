...
(function (factory) {
  ...
  var defaults = {
    ...
    options: {
	  ...
	  //@moditha. enables template parsing
	  templateEditor:true,			
	  parserRegEx:"@View\\w+.\\w+", 
	  dataBackup:null,
      ...
      icons: {
        ...
        misc: {
          check: 'check',
		  save: 'floppy-o', //@moditha
		  reset: 'fa-undo' //@moditha
        }
      },
      ...
      toolbar: [
        ...
        ['help', ['help']],
        ['template', ['tags', 'savetemplate', 'resettemplate']] //@moditha
      ],
  ...
  var Typing = function () {
    ...
    this.insertParagraph = function () {
      ...
      rng = rng.wrapBodyInlineWithPara();
        ...
        //@moditha
        //do nothing within tags
      if (rng.sc.parentNode.classList.contains("template-tag"))
          return;
  ...
  var EventHandler = function () {
    ...
    var commands = {
	  ...
	  //----- @moditha
      saveTemplate: function (layoutInfo) {
		  var $editable = layoutInfo.editable();
		  var callbacks = $editable.data('callbacks');
			if ($.isFunction(callbacks.onSave)){
				var iTags = $(".note-editable i.tag-placeholder" );
				iTags.unwrap();
				$(".note-editable i.tag-placeholder").remove();				
				callbacks.onSave($editable.code());
			}

      },
	  

      resetTemplate: function (layoutInfo) {
		  var $editable = layoutInfo.editable();
		  var callbacks = $editable.data('callbacks');
			if ($.isFunction(callbacks.onReset)){
				//$editable.html($.summernote.options.dataBackup,true);
				callbacks.onReset();
			}
      },
      tags: function (layoutInfo) {
      },
	  //-------@moditha
      /**
       * @param {Object} layoutInfo
    ...
    this.attach = function (layoutInfo, options) {
      ...
      layoutInfo.editable().data('callbacks', {
		...
		onSave : options.onSave, //@moditha
		onReset : options.onReset //@moditha
      });
  ...
  var Renderer = function () {
    ...
    var tplButton = function (label, options) {
	  ...
	  var text = options.text ? ' ' + options.text : ''; //@moditha
	  var btnClass = options.btnClass ? ' ' + options.btnClass : ''; //@moditha
	  
      return (dropdown ? '<div class="btn-group' +
               (className ? ' ' + className : '') + '">' : '') +
               '<button type="button"' +
                 ' class="btn btn-default btn-sm' + btnClass + //@moditha
				 ...
				 (text ? text + '</button>' : '</button>') + //@moditha
               //'</button>' + //@moditha commented
               (dropdown || '') +
             (dropdown ? '</div>' : '');
    };
    ...
    var tplButtonInfo = {
	  ...
	  //start - @moditha
      savetemplate: function (lang, options){ 
        return tplIconButton(options.iconPrefix + options.icons.misc.save, {
          event: 'saveTemplate',
          title: 'Save Template',
		  text: 'Save Template',
      ...
      resettemplate: function (lang, options) {
        return tplIconButton(options.iconPrefix + options.icons.misc.reset, {
          event: 'resetTemplate',
      ...
      tags: function (lang, options) {
        var label = '<span class="note-tags">Tags</span>';
		var items = [];
        return tplButton(label, {
		  event: 'tags',
          title: 'Tags',
          className: 'btn-default',
          dropdown: tplDropdown(items, 'tags')
        });	
      },
	  //end - @moditha
  ...
  $.fn.extend({
     ...
     */
    code: function (html, parse) { //@moditha - added parseTemplate
	  ...
	  //@moditha
	  if (parse && html && $.summernote.options.templateEditor){ 
	  
		$('#tags').empty();
		
		var templateListItem = "<li><a href='#' class='tag'>|</a></li>";
		var templateLabel = "<span contentEditable='false' data-toggle='tooltip' class='btn btn-info btn-sm template-tag' title='Click trash icon to remove this tag' ><i class='fa fa-trash fa- tag-placeholder'>&nbsp;</i>|</span>";
		
		var re = new RegExp($.summernote.options.parserRegEx, 'g');
		...
		if (matches.length > 0) {
		    ...
		    for (var i = 0; i < matches.length; i++) {
		        $('.tags').append(templateListItem.replace("|", matches[i]));
		        endIndex = html.indexOf(matches[i]) + matches[i].length;
		        temp = html.substring(startIndex, endIndex);
		        html = html.substr(endIndex, html.length);
		        parsedHtml += temp.replace(matches[i], templateLabel.replace("|", matches[i]));

		        //html = html.replace(matches[i], templateLabel.replace("|", matches[i]));
			}
			
			$('.note-editor').on("click",".tag-placeholder",
				function () {
			...
			$(".tag").click(function () {
			    var placeholder = templateLabel.replace('|', $(this).text());
			    var $dom = $(placeholder);
