...
(function (factory) {
  ...
  var defaults = {
    ...
    options: {
	  ...
	  templateEditor:true,			//@moditha. enables template parsing
	  
	  parserRegEx:"@View\\w+.\\w+",//@moditha. enables template parsing	  
      ...
      saveTemplate: function (layoutInfo) {
		  ...
		  var callbacks = $editable.data('callbacks');
			if ($.isFunction(callbacks.onSave)){
				var dataDOM = $($editable.code());
				dataDOM.find("i" ).unwrap().remove();
				//callbacks.onSave($editable.code());
				callbacks.onSave(dataDOM.html());
			}
  ...
  $.fn.extend({
    ...
    code: function (html, parse) { //@moditha - added parseTemplate
	  ...
	  if (parse && html && $.summernote.options.templateEditor){ 
		...
		var templateLabel = "<span contentEditable='false' data-toggle='tooltip' class='btn btn-info btn-sm template-tag' title='Click to remove tag' ><i class='fa fa-trash fa- tag-placeholder'>&nbsp;</i>|</span>&nbsp;";
		
		//var matches = html.match(/@View\w+.\w+/g);
